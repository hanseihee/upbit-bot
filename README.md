...
소액(10~50만원) 투자에 최적화되었으며, **승률 70~85%** 를 목표로 설계되었다.
...
```
현재가: 90,000,000원 / ATR: 2,000,000 / 간격 = ATR × 0.5 = 1,000,000

 매도 L1: 91,000,000  ←  L1 매수 체결 시 여기서 매도
 매도 L2: 90,000,000  ←  L2 매수 체결 시 여기서 매도
━━━━━━━━━━━━━━━━━━━━  현재가
 매수 L1: 89,000,000  ←  1차 매수
 매수 L2: 88,000,000  ←  2차 매수
 매수 L3: 87,000,000  ←  3차 매수
 매수 L4: 86,000,000  ←  4차 매수
 매수 L5: 85,000,000  ←  5차 매수
━━━━━━━━━━━━━━━━━━━━
 손절가:  80,750,000   (L5 기준 -5%)
```
...
├── strategy/
│   ├── indicators.py        # 기술적 지표 (RSI, BB, ATR, MACD)
│   ├── signals.py           # 매매 신호 생성 + 신뢰도 계산
...
**실전 투입 전 반드시 Paper 모드로 최소 1주일 테스트할 것.**

```bash
python main.py --mode paper
```

- 초기 가상 잔고: 500,000 KRW
- 실제 주문을 전송하지 않음
- 현재가 도달 시 가상 체결 처리
- 로그에 `[PAPER]` 접두사 표시
...
```bash
# 기본: KRW-BTC 90일, 잔고 50만원
python -m backtest.engine

# 커스텀 설정
python -m backtest.engine --market KRW-ETH --days 60 --balance 300000
```
...
| 설정 | 기본값 | 설명 | 조정 가이드 |
|------|--------|------|-------------|
| `grid_levels` | 5 | 그리드 단계 수 | 소액: 3~5, 대규모: 7~10 |
| `grid_spacing_atr_mult` | 0.5 | ATR 배수 (간격) | 높이면 안전, 낮추면 거래 빈도 증가 |
| `rsi_period` | 14 | RSI 계산 기간 | 표준값. 변경 비권장 |
| `rsi_oversold` | 30.0 | 과매도 기준 | 낮추면 진입 줄어들고 승률 상승 |
| `rsi_overbought` | 70.0 | 과매수 기준 | 높이면 청산 지연, 수익 기회 |
| `bb_period` | 20 | 볼린저밴드 기간 | 표준값 |
| `bb_std` | 2.0 | 볼린저밴드 표준편차 | 높이면 진입 줄어들고 더 극단적 |
| `candle_unit` | 15 | 분봉 단위 | 5분: 빈번, 60분: 느림, 15분 권장 |

### 코인 선별 기준

| 설정 | 기본값 | 설명 |
|------|--------|------|
| `max_coins` | 2 | 동시 운용 코인 수. 소액은 2개 이하 권장 |
| `min_volume_krw` | 10억 | 24h 최소 거래대금. 유동성 보장 |
| `max_daily_change` | 10% | 일일 변동률 상한. 초과 시 제외 |
| `max_spread_pct` | 0.3% | 호가 스프레드 상한. 체결 비용 제한 |
| `coin_reselect_hours` | 4 | 코인 재선별 주기 |
...
| `stop_loss_pct` | 5% | 그리드 하단 기준 손절 | 높음 |
| `max_drawdown_pct` | 10% | 총자산 드로다운 한도 (초과 시 전체 청산) | 최고 |
| `max_position_age_hours` | 48 | 포지션 최대 유지 시간 | 중간 |
| `min_order_krw` | 5,000 | 최소 주문금액 (Upbit 규정) | 고정 |

### 소액 최적화 요점

1. **그리드 5단계**: 잔고 30만원 기준 레벨당 ~24,000원. 최소주문 5,000원 충족
2. **최대 2코인**: 너무 분산하면 레벨당 금액이 최소주문 미달
3. **15분봉**: 5분봉은 노이즈 과다, 1시간봉은 기회 놓침
4. **수수료 고려**: Upbit 0.05%. 그리드 간격이 수수료 × 3 이상 확보 (자동)
5. **시장가 매도**: 소액은 슬리피지가 미미하므로 긴급 시 시장가 사용
...
### 기술적 한계
- **폴링 방식**: 현재 60초 간격 폴링. 급변 시 최대 60초 지연
